...
- Solution
  ...
  - SCSS MAP
    ...
    ```
      @import '@ngx-app-frame/theme/styles/component';
...
Documentation is generated by the custom generator built on top of @ngx-app-frame/theme.
...
1. `rm -rf node_modules/@ngx-app-frame` to remove the package installed
2. run `npm link` for each ngx-app-frame module in *ngx-app-frame/src/framework* except for *icons*
3. make sure you don't have *node_modules* in ngx-app-frame project (this may cause an issue an issue the angular-cli)
4. link ngx-app-frame in ngx-admin: `npm link @ngx-app-frame/{auth,theme}`
