...
- src
    ...
    - framework - Framework itself, divided into npm packages
        - theme - `@oliveui/theme` npm package, main framework package
        - auth - `@oliveui/auth` npm package, auth package (login, register, etc)       
        - icons - `oliveui-icons` npm package, cool icons font
        - security - `@oliveui/security` npm package, security framework package
...
- Solution
  ...
  - SCSS MAP
    ...
    ```
      @import '@oliveui/theme/styles/component';
...
Documentation is generated by the custom generator built on top of @oliveui/theme.
...
#ngx-admin development on the latest oliveui sources

1. `rm -rf node_modules/@oliveui` to remove the package installed
2. run `npm link` for each oliveui module in *oliveui/src/framework* except for *icons*
3. make sure you don't have *node_modules* in oliveui project (this may cause an issue an issue the angular-cli)
4. link oliveui in ngx-admin: `npm link @oliveui/{auth,theme}`
